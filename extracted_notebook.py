...
for year, df in zip(years, datasets, strict=False):
...
df_2015_std = df_2015_std.rename(
    columns={
        "Country": "Country",
...
df_2016_std = df_2016_std.rename(
    columns={
        "Country": "Country",
        "Happiness Rank": "Happiness_Rank",
        "Happiness Score": "Happiness_Score",
...
df_2017_std = df_2017_std.rename(
    columns={
        "Country": "Country",
        "Happiness.Rank": "Happiness_Rank",
        "Happiness.Score": "Happiness_Score",
...
df_2018_std = df_2018_std.rename(
    columns={
...
df_2019_std = df_2019_std.rename(
    columns={
        "Country or region": "Country",
        "Overall rank": "Happiness_Rank",
        "Score": "Happiness_Score",
...
df_2020_std = df_2020_std.rename(
    columns={
...
df_2021_std = df_2021_std.rename(
    columns={
        "Country name": "Country",
        "Happiness Rank": "Happiness_Rank",
        "Happiness score": "Happiness_Score",
...
df_2022_std = df_2022_std.rename(
    columns={
        "Country name": "Country",
        "Happiness Rank": "Happiness_Rank",
        "Happiness score": "Happiness_Score",
...
df_2023_std = df_2023_std.rename(
    columns={
        "Country name": "Country",
        "Happiness Rank": "Happiness_Rank",
        "Happiness score": "Happiness_Score",
...
df_2024_std = df_2024_std.rename(
    columns={
        "Country name": "Country",
        "Happiness Rank": "Happiness_Rank",
        "Happiness score": "Happiness_Score",
...
for year, df in zip(years, standardized_dfs, strict=False):
...
for year, df in zip(years, standardized_dfs, strict=False):
    for column in df.columns:
        if df[column].isnull().sum() > 0:
            ...
            print(
                f"Filled missing values in {year} dataset, column '{column}' with median: {median_value:.4f}"
            )
...
sns.lineplot(
    x="Year", y="Happiness_Score", data=yearly_avg, marker="o", linewidth=2.5, color="#1f77b4"
)
...
# Add value labels
for x, y in zip(yearly_avg["Year"], yearly_avg["Happiness_Score"], strict=False):
...
sns.barplot(
    ...
    y="Country",
    x="Happiness_Score",
    data=top10.sort_values("Happiness_Score", ascending=False),
    palette="viridis",
)
...
sns.barplot(
    ...
    y="Country",
    x="Happiness_Score",
    data=bottom10.sort_values("Happiness_Score"),
    palette="viridis",
)
...
for country in top5_countries:
    ...
    for x, y in zip(country_data["Year"], country_data["Happiness_Rank"], strict=False):
...
for year in years:
    ...
    for factor in [
        ...
        correlation_data.append({"Year": year, "Factor": factor, "Correlation": corr})
...
for i, factor in enumerate(factors):
    ...
    axes[i].annotate(
        ...
        fontsize=12,
        bbox={"boxstyle": "round,pad=0.3", "fc": "white", "ec": "b", "lw": 1},
    )
...
covid_impact_melted = covid_impact_df.melt(
    id_vars="Factor", var_name="Period", value_name="Average Value"
)

# Create the plot
plt.figure(figsize=(16, 10))
sns.barplot(
    x="Factor", y="Average Value", hue="Period", data=covid_impact_melted, palette="viridis"
)
...
region_happiness = (
    df_2024_regions.groupby("Region")["Happiness_Score"]
...
print("ANOVA Results for Happiness Scores Across Years (2015-2024):")
...
print("Independent t-test Results for Pre-COVID vs Post-COVID Happiness:")
...
X = df_2024_data[
    ["GDP_per_Capita", "Social_Support", "Life_Expectancy", "Freedom", "Corruption", "Generosity"]
]
...
periods = {
    ...
    "Pre-COVID (2018-2019)": df_combined[
        (df_combined["Year"] >= 2018) & (df_combined["Year"] <= 2019)
    ],
    "COVID (2020-2022)": df_combined[(df_combined["Year"] >= 2020) & (df_combined["Year"] <= 2022)],
    "Recent (2023-2024)": df_combined[
        (df_combined["Year"] >= 2023) & (df_combined["Year"] <= 2024)
    ],
}
...
# Display results
pivot_table = model_comparison_df.pivot_table(
    index="Factor", columns="Period", values="Coefficient"
)
display(pivot_table)
...
results_df = pd.DataFrame(
    {"Actual": y_test, "Linear Regression": y_pred_lr, "Random Forest": y_pred_rf}
...
plt.scatter(
    range(len(results_df)), results_df["Actual"], color="blue", label="Actual", s=100, alpha=0.7
)
plt.scatter(
...
X_train_temporal = pre_covid[
    ["GDP_per_Capita", "Social_Support", "Life_Expectancy", "Freedom", "Corruption", "Generosity"]
]
y_train_temporal = pre_covid["Happiness_Score"]

X_test_temporal = post_covid[
    ["GDP_per_Capita", "Social_Support", "Life_Expectancy", "Freedom", "Corruption", "Generosity"]
]
