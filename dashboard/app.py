...
selected_year = st.sidebar.select_slider("Select Year", options=available_years, value=max(available_years) if available_years else None)

regions = st.sidebar.multiselect("Select Countries (Optional)", options=sorted(df['country_name'].unique().tolist()))
...
with col1:
    ...
    st.metric(f"Avg Happiness ({selected_year})", f"{avg_happiness:.2f}" if pd.notnull(avg_happiness) else "N/A")
with col2:
    avg_gdp = filtered_df['gdp_per_capita'].mean()
    st.metric(f"Avg GDP Per Capita", f"${avg_gdp:,.0f}" if pd.notnull(avg_gdp) else "N/A")
with col3:
    avg_co2 = filtered_df['co2_per_capita'].mean()
    st.metric(f"Avg CO2 Per Capita", f"{avg_co2:.2f} t" if pd.notnull(avg_co2) else "N/A")
with col4:
...
if not df.empty:
    trend_df = df.groupby('year')[['happiness_score', 'gdp_per_capita', 'co2_per_capita']].mean().reset_index()
    fig_trend = px.line(
        trend_df, 
        x="year", 
        y="happiness_score",
